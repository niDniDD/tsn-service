...
!function(app){"use strict";app.registerModule("news")}(ApplicationConfiguration);
!function(app){"use strict";app.registerModule("teams")}(ApplicationConfiguration);
...
!function(){"use strict";function DownloadsListController(DownloadsService){var vm=this;vm.downloads=DownloadsService.query()}angular.module("downloads").controller("DownloadsListController",DownloadsListController),DownloadsListController.$inject=["DownloadsService"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("topbar",{title:"News",state:"news",type:"dropdown",roles:["*"]}),menuService.addSubMenuItem("topbar","news",{title:"List News",state:"news.list"}),menuService.addSubMenuItem("topbar","news",{title:"Create News",state:"news.create",roles:["user"]})}angular.module("news").run(menuConfig),menuConfig.$inject=["Menus"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("news",{"abstract":!0,url:"/news",template:"<ui-view/>"}).state("news.list",{url:"",templateUrl:"modules/news/client/views/list-news.client.view.html",controller:"NewsListController",controllerAs:"vm",data:{pageTitle:"News List"}}).state("news.create",{url:"/create",templateUrl:"modules/news/client/views/form-news.client.view.html",controller:"NewsController",controllerAs:"vm",resolve:{newsResolve:newNews},data:{roles:["user","admin"],pageTitle:"News Create"}}).state("news.edit",{url:"/:newsId/edit",templateUrl:"modules/news/client/views/form-news.client.view.html",controller:"NewsController",controllerAs:"vm",resolve:{newsResolve:getNews},data:{roles:["user","admin"],pageTitle:"Edit News {{ newsResolve.name }}"}}).state("news.view",{url:"/:newsId",templateUrl:"modules/news/client/views/view-news.client.view.html",controller:"NewsController",controllerAs:"vm",resolve:{newsResolve:getNews},data:{pageTitle:"News {{ newsResolve.name }}"}})}function getNews($stateParams,NewsService){return NewsService.get({newsId:$stateParams.newsId}).$promise}function newNews(NewsService){return new NewsService}angular.module("news").config(routeConfig),routeConfig.$inject=["$stateProvider"],getNews.$inject=["$stateParams","NewsService"],newNews.$inject=["NewsService"]}();
!function(){"use strict";function NewsListController(NewsService){var vm=this;vm.news=NewsService.query()}angular.module("news").controller("NewsListController",NewsListController),NewsListController.$inject=["NewsService"]}();
!function(){"use strict";function NewsController($scope,$state,$window,Authentication,news){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.news.$remove($state.go("news.list"))}function save(isValid){function successCallback(res){$state.go("news.view",{newsId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.news._id?vm.news.$update(successCallback,errorCallback):vm.news.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.newsForm"),!1)}var vm=this;vm.authentication=Authentication,vm.news=news,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("news").controller("NewsController",NewsController),NewsController.$inject=["$scope","$state","$window","Authentication","newsResolve"]}();
!function(){"use strict";function NewsService($resource){return $resource("api/news/:newsId",{newsId:"@_id"},{update:{method:"PUT"}})}angular.module("news").factory("NewsService",NewsService),NewsService.$inject=["$resource"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("topbar",{title:"Teams",state:"teams",type:"dropdown",roles:["*"]}),menuService.addSubMenuItem("topbar","teams",{title:"List Teams",state:"teams.list"}),menuService.addSubMenuItem("topbar","teams",{title:"Create Team",state:"teams.create",roles:["user"]})}angular.module("teams").run(menuConfig),menuConfig.$inject=["Menus"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("teams",{"abstract":!0,url:"/teams",template:"<ui-view/>"}).state("teams.list",{url:"",templateUrl:"modules/teams/client/views/list-teams.client.view.html",controller:"TeamsListController",controllerAs:"vm",data:{pageTitle:"Teams List"}}).state("teams.create",{url:"/create",templateUrl:"modules/teams/client/views/form-team.client.view.html",controller:"TeamsController",controllerAs:"vm",resolve:{teamResolve:newTeam},data:{roles:["user","admin"],pageTitle:"Teams Create"}}).state("teams.edit",{url:"/:teamId/edit",templateUrl:"modules/teams/client/views/form-team.client.view.html",controller:"TeamsController",controllerAs:"vm",resolve:{teamResolve:getTeam},data:{roles:["user","admin"],pageTitle:"Edit Team {{ teamResolve.name }}"}}).state("teams.view",{url:"/:teamId",templateUrl:"modules/teams/client/views/view-team.client.view.html",controller:"TeamsController",controllerAs:"vm",resolve:{teamResolve:getTeam},data:{pageTitle:"Team {{ teamResolve.name }}"}})}function getTeam($stateParams,TeamsService){return TeamsService.get({teamId:$stateParams.teamId}).$promise}function newTeam(TeamsService){return new TeamsService}angular.module("teams").config(routeConfig),routeConfig.$inject=["$stateProvider"],getTeam.$inject=["$stateParams","TeamsService"],newTeam.$inject=["TeamsService"]}();
!function(){"use strict";function TeamsListController(TeamsService){var vm=this;vm.teams=TeamsService.query()}angular.module("teams").controller("TeamsListController",TeamsListController),TeamsListController.$inject=["TeamsService"]}();
!function(){"use strict";function TeamsController($scope,$state,$window,Authentication,team){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.team.$remove($state.go("teams.list"))}function save(isValid){function successCallback(res){$state.go("teams.view",{teamId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.team._id?vm.team.$update(successCallback,errorCallback):vm.team.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.teamForm"),!1)}var vm=this;vm.authentication=Authentication,vm.team=team,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("teams").controller("TeamsController",TeamsController),TeamsController.$inject=["$scope","$state","$window","Authentication","teamResolve"]}();
!function(){"use strict";function TeamsService($resource){return $resource("api/teams/:teamId",{teamId:"@_id"},{update:{method:"PUT"}})}angular.module("teams").factory("TeamsService",TeamsService),TeamsService.$inject=["$resource"]}();
...
"use strict";angular.module("users").controller("AuthenticationController",["$scope","$state","$http","$location","$window","Authentication","PasswordValidator",function($scope,$state,$http,$location,$window,Authentication,PasswordValidator){$scope.authentication=Authentication,$scope.popoverMsg=PasswordValidator.getPopoverMsg(),$scope.error=$location.search().err,$scope.authentication.user&&$location.path("/"),$scope.signup=function(isValid){$scope.error=null,$http.post("/api/auth/signup",$scope.credentials).success(function(response){$scope.authentication.user=response,$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message})},$scope.signin=function(isValid){return $scope.error=null,isValid?void $http.post("/api/auth/signin",$scope.credentials).success(function(response){$scope.authentication.user=response,$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message}):($scope.$broadcast("show-errors-check-validity","userForm"),!1)},$scope.callOauthProvider=function(url){$state.previous&&$state.previous.href&&(url+="?redirect_to="+encodeURIComponent($state.previous.href)),$window.location.href=url}}]);
